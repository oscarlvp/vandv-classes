...
Prime path coverage::
A _simple path_ in a graph is a path where no node appears more than once, except perhaps the first node, that could also be the last node. A _prime path_ in a graph is a simple path that is no proper sub-path of any other. That is a prime path is a simple maximal path. 

// TODO: Explain in the example

////
TODO: Explore the relationship with linearly independent paths
Prime paths can be combined to form any other path over the control flow graph. 
////  


*Prime path coverage* sets all prime paths as test requirements. It helps make meaningful path combinations while keeping the number of tests low.

However, while prime paths form all paths in the graph, not all their combinations produce a valid test path.
