...
These blocks may be too broad for testing purposes. Sometimes it is useful partition blocks into sub-partitions. This is specially useful for valid values. As the validity of `month` and `day` depend on each other and the value of `year` we turn to a functionality based approach.

|===
| Characteristics  3+| Blocks 
| Value of `year`    | <= 0 | 0 | valid leap year               | valid non leap year
| Value of `month`   | <= 0 | 0 | between 1 and 12              | > 12
| Value of `day`     | <= 0 | 0 | between 1 and max per `month` | > max per `month` 
|===
