...
[source, bash]
....
<path-to-pmd-folder>/bin/run.sh pmd -d <path-to-java-file> -f text -R <path-to-rule> 
....

The line above runs PMD over a single Java file using a single rule and outputs the result in plain text in the console.
...
Example with CloseResource, highlight the limitations.
Show how to create a rule with Xpath and how to implement one in Java.
Use number of public methods as example for the metric.
////
