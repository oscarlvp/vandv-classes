...
Tools like PMD search for patterns in the source code and these patterns may not include all cases, as seen in the two examples above. This is also a limitation of static analysis in general. The code is not executed, therefore therefore no dynamic behavior is considered, as in the case of <<stream-example>> where the issue is influenced by the input of the user.
