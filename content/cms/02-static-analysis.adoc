...
Classes with low coupling are easier to reuse. Classes with large coupling should be avoided and refactored. If a class with a large coupling becomes necessary, then these it requires rigorous testing to correctly verify its interaction with its dependencies. A study from 2010 performed on the Eclipse project concluded that, among other metrics, CBO is a significant predictor on how prone a class is to bugs. The same study says that a CBO greater than 9 signals a high risk to introduce a fault.
